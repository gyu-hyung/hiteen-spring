...
class PollController(
    ...
    @Operation(
        summary = "투표 목록",
        description = "커서 기반 페이지네이션 방식으로 투표 목록을 조회합니다. type 파라미터로 mine(내가 등록한), participated(내가 참여한) 필터링 가능. orderType=DISTANCE로 거리순 정렬 가능."
    )
    @GetMapping
    suspend fun list(
        ...
        @Parameter(description = "정렬 타입: LATEST(기본), POPULAR, LIKE, COMMENT, DISTANCE") @RequestParam(required = false) orderType: String?,
        ...
        val isDistanceOrder = orderType?.uppercase() == "DISTANCE"

        // 거리순 정렬 시 커서 파싱 (거리:ID 형식)
        val (lastDistance, lastId) = if (isDistanceOrder && cursor != null) {
            cursor.split(":").let {
        ...
        // 기본 정렬 시 커서는 Long ID
        val cursorId = if (!isDistanceOrder && cursor != null) cursor.toLongOrNull() else null

        val list = service.listPollsByCursor(
            cursorId, size + 1, user?.id, type, author, orderType,
            lat, lng, maxDistance, lastDistance, lastId
        )
