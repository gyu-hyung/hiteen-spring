...
class ArticleController(
    ...
    @Operation(
        ...
        description = "카테고리(NOTICE/EVENT), 검색어, 페이지 기반 페이지네이션으로 목록을 조회합니다."
    )
    @GetMapping
    suspend fun listByPage(
        ...
        val result = service.listArticlesByPage(category?.name, q, page, size)
        return ResponseEntity.ok(ApiResult.success(result))
    }

    @Operation(
        summary = "공지사항/이벤트 목록 조회 (커서 기반)",
        description = "카테고리(NOTICE/EVENT), 검색어, 커서 기반 페이지네이션으로 목록을 조회합니다."
    )
    @GetMapping("/cursor")
    suspend fun listByCursor(
        ...
        val result = service.listArticlesByCursor(category?.name, q, size, cursor)
        return ResponseEntity.ok(ApiResult.success(result))
    }

    @Operation(summary = "공지사항/이벤트 단건 조회", description = "특정 공지사항/이벤트를 ID로 조회합니다.")
    @GetMapping("/{id}")
    suspend fun get(
        @Parameter(description = "게시글 ID") @PathVariable id: Long,
        @AuthenticationPrincipal(expression = "user") user: UserEntity?,
    ): ResponseEntity<ApiResult<ArticleResponse>> {
